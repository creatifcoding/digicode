...
#
# Paths after install:
# - ~/.jcode/builds/versions/<hash>/digicode (immutable primary executable)
# - ~/.jcode/builds/versions/<hash>/jcode -> digicode (compatibility alias)
# - ~/.jcode/builds/stable/digicode -> .../versions/<hash>/digicode
# - ~/.jcode/builds/current/digicode -> .../versions/<hash>/digicode
# - ~/.local/bin/digicode -> ~/.jcode/builds/current/digicode (launcher)
set -euo pipefail
...
cargo build --profile "$profile" --manifest-path "$repo_root/Cargo.toml" --bin digicode
bin="$repo_root/target/$profile/digicode"
...
mkdir -p "$version_dir"
install -m 755 "$bin" "$version_dir/digicode"
ln -sfn digicode "$version_dir/jcode"

# Update stable symlink
stable_dir="$builds_dir/stable"
mkdir -p "$stable_dir"
ln -sfn "$version_dir/digicode" "$stable_dir/digicode"
...
mkdir -p "$current_dir"
ln -sfn "$version_dir/digicode" "$current_dir/digicode"
...
mkdir -p "$install_dir"
ln -sfn "$current_dir/digicode" "$install_dir/digicode"
ln -sfn "$current_dir/jcode" "$install_dir/jcode"

echo "Installed: $version_dir/digicode"
echo "Compatibility alias: $version_dir/jcode -> digicode"
echo "Updated stable symlink: $stable_dir/digicode -> $version_dir/digicode"
echo "Updated current symlink: $current_dir/digicode -> $version_dir/digicode"
echo "Updated launcher symlink: $install_dir/digicode -> $current_dir/digicode"
echo "Updated compatibility launcher: $install_dir/jcode -> $current_dir/jcode"
...
case "$(uname -s)" in
  Darwin)
    if "$install_dir/digicode" setup-launcher </dev/null >/dev/null 2>&1; then
      echo "Installed macOS launcher and turn-notification broker."
    fi
    if "$install_dir/digicode" setup-hotkey </dev/null >/dev/null 2>&1; then
...
if [ "${JCODE_SKIP_SERVER_RELOAD:-}" != "1" ]; then
  if "$install_dir/digicode" server reload </dev/null >/dev/null 2>&1; then
