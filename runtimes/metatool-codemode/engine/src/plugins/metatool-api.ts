 ...
 *   ms.runUtil('full-health', 'metatool')
...
export function createApi(cwd: string) {
  ...
  const metatoolDir = join(skillsDir, 'metatool')
  ...
  function inspect(name: string): HealthReport {
    ...
    checks.push(check('governance',
      readHead(join(dir, 'SKILL.md')).includes('governed-by: metatool'),
  ...
  function protocol(key: string): string | null {
    const skillMd = join(metatoolDir, 'SKILL.md')
  ...
  function utils(): UtilInfo[] {
    const dir = join(metatoolDir, 'utils')
  ...
  function runUtil(utilName: string, skillName: string): UtilResult {
    const utilFile = join(metatoolDir, 'utils', `${utilName}.md`)
  ...
  function adopt(name: string): string {
    ...
    setFrontmatter(skillMd, 'governed-by', 'metatool')
