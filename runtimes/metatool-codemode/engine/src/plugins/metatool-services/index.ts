/**
 * @module plugins/metatool-services
 *
 * Effect v4 Context.Service decomposition of the metatool domain.
 *
...
export { JournalService, JournalServiceLive, ENRICHMENT_MARKER, parseJournalEntries } from "./journal-service.js"
export type {
  JournalServiceShape,
  JournalEntry,
  AppendJournalEntryInput,
  AppendJournalEnrichmentInput,
  JournalAppendEvidence,
...
export {
  ...
  ProtocolNotFound, UtilNotFound, ExecutionError, JournalError,
  MetatoolError,
} from "./errors.js"
export type {
  SkillNotFound as SkillNotFoundType,
  FileReadError as FileReadErrorType,
  MetatoolError as MetatoolErrorType,
} from "./errors.js"
...
import { JournalServiceLive } from "./journal-service.js"


/**
 * Build the full metatool service layer graph for a given cwd.
 *
 ...
 * const layer = makeMetatoolLayer("/path/to/project")
 * const runtime = ManagedRuntime.make(layer.pipe(Layer.provide(NodeFileSystem.layer)))
 * ```
 */
export function makeMetatoolLayer(cwd: string) {
  ...
  const journalLayer = JournalServiceLive.pipe(Layer.provide(configLayer))
