 ...
 * @module overlay
 *
 * MetatoolOverlay — the composable surface contract.
 *
 * An overlay reshapes the entire metatool tool surface:
 ...
 *
 * Replaces the earlier MetatoolPlugin interface.
 *
 * @example
 * ```ts
 * const metatool: MetatoolOverlay = {
 *   id: "metatool",
 *   name: "Skill Governance",
 *   methods: { discover: () => ..., inspect: (n) => ... },
 *   guide: { sections: [{ id: "metatool-ops", slot: "api", priority: 20, content: "..." }] },
...
import type { MetatoolCore } from "./types.js"

// ── MetatoolOverlay Interface ────────────────────────────────────

export interface MetatoolOverlay {
  ...
  readonly lifecycle?: {
    ...
    readonly onLoad?: (core: MetatoolCore) => void | Promise<void>
...
export class OverlayManager {
  private stack: MetatoolOverlay[] = []
  private _compiled: CompiledOverlayState = emptyCompiled()
  private core: MetatoolCore | null = null
  ...
  setCore(core: MetatoolCore): void {
    this.core = core
  }

  /** Load an overlay onto the top of the stack */
  async load(overlay: MetatoolOverlay): Promise<void> {
  ...
  async loadBatch(overlays: ReadonlyArray<MetatoolOverlay>): Promise<void> {
  ...
  async switchTo(overlay: MetatoolOverlay): Promise<void> {
    await this.clear()
  ...
  get(id: string): MetatoolOverlay | undefined {
  ...
  private async seedProcedures(overlay: MetatoolOverlay): Promise<void> {
    ...
    for (const procedure of overlay.procedures) {
      ...
      const unchanged = existing !== null
        && existing.code === code
        && existing.manifest === procedure.manifest
 ...
 */
function compile(stack: ReadonlyArray<MetatoolOverlay>): CompiledOverlayState {
...
/**
 * Convert a legacy MetatoolPlugin to an Overlay.
 * Preserves all existing plugin fields, maps them to overlay shape.
 *
 * @deprecated Use MetatoolOverlay directly.
 */
export function pluginToOverlay(plugin: {
  ...
  setup?: (core: MetatoolCore) => void | Promise<void>
