...
export const evalDiscipline: SectionConfig = {
  ...
  content: [
    '## SKILL GOVERNANCE (overlay: metatool — all async, use await)',
    '',
    'DISCOVERY (async):',
    ...
    '  Only cwd-contained *.journal.md files and grandfathered infra-journal.md are valid.',
    '',
...
export const overlayManagement: SectionConfig = {
  ...
  content: [
    ...
    '  cm.metatool()        → MetatoolOverlay (skill governance)',
    '',
    'EXAMPLE:',
    '  await cm.loadOverlay(cm.metatool())   // load skill governance',
    '  cm.overlays()                           // → [{id:"metatool", name:"Skill Governance"}]',
    '  const skills = await cm.discover()     // now available',
    '  await cm.unloadOverlay("metatool")    // unload',
  ].join('\n'),
}
...
export const usagePatterns: SectionConfig = {
  ...
  content: [
    ...
    '  OVERLAY: await cm.loadOverlay(cm.metatool()) — load on demand',
  ].join('\n'),
}
...
export const antiPatterns: SectionConfig = {
  ...
  content: [
    ...
    '  - Assuming overlays are loaded — check cm.hasOverlay("metatool") first',
