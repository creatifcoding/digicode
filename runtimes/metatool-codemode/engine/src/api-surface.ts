...
export interface ApiMethod {
  ...
  signature: string
  /** Source module: 'core' | 'store' | 'builder' | 'domain' | 'context' | 'llm' | 'procedure' | 'overlay' | 'journal' */
  source: 'core' | 'store' | 'builder' | 'domain' | 'context' | 'llm' | 'procedure' | 'overlay' | 'journal'
 ...
 *   properties (context), in-memory reads (history), factories (metatool),
 *   and overlay queries (overlays, hasOverlay).
 */
export const API_SURFACE: ApiMethod[] = [
  // ── Core overlay methods (populated by metatool overlay) ──
  // ALL metatool methods are async (Effect.runPromise at boundary)
  ...
  // ── Journals (append-only, async) ──────────────────────
  { name: 'journalEntries',          section: 'eval-discipline', signature: 'async (path) → JournalEntry[]',                       source: 'journal', color: 'async' },
  { name: 'appendJournalEntry',      section: 'eval-discipline', signature: 'async (path, { id, date, title, author, context, body }) → JournalAppendEvidence', source: 'journal', color: 'async' },
  { name: 'appendJournalEnrichment', section: 'eval-discipline', signature: 'async (path, entryId, { date, agent, body }) → JournalAppendEvidence',               source: 'journal', color: 'async' },
  ...
  { name: 'metatool',    section: 'overlay-management', signature: '() → MetatoolOverlay',                        source: 'overlay', color: 'sync' },
]
