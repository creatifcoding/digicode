 ...
 * import { sqliteNodeLayer } from "@tmnl/metatool/adapters/sqlite-node"
 * import { metatoolOverlay } from "@tmnl/metatool/plugins/metatool"
 *
 * const metatool = await createMetatool({
 *   sqlLayer: sqliteNodeLayer({ filename: ".pi/rlm/store.db" }),
 *   overlays: [metatoolOverlay(cwd, fsLayer)],
 *   cwd: process.cwd(),
 * })
 *
 * const result = await metatool.eval('await ms.discover()')
...
import { MetatoolRuntime } from "./services/MetatoolRuntime.js"
import type { RuntimeCommand, RuntimeExecOptions } from "./schemas/runtime.js"
...
import { OverlayManager, pluginToOverlay, type MetatoolOverlay, type CompiledOverlayState } from "./overlay.js"
import type { MetatoolCore, MetatoolConfig, MetatoolInstance } from "./types.js"

// Legacy compat
import type { MetatoolPlugin } from "./plugin.js"

// ── Options ──────────────────────────────────────────────────────

export interface CreateMetatoolOptions {
  /** Effect Layer providing SqlClient — pick your backend */
  readonly sqlLayer: Layer.Layer<SqlClient>

  /** Effect Layer providing FileSystem — needed by the default Node runtime and store import/export */
  readonly fsLayer: Layer.Layer<FileSystem>

  /** Effect Layer providing execution/filesystem runtime; defaults to Node compatibility */
  readonly runtimeLayer?: Layer.Layer<MetatoolRuntime, any, FileSystem>

  /** Overlays to load (preferred — full surface control) */
  readonly overlays?: ReadonlyArray<MetatoolOverlay>

  /**
   * Legacy plugin support — auto-wrapped to overlays.
   * @deprecated Use `overlays` instead.
   */
  readonly plugins?: ReadonlyArray<MetatoolPlugin>
...
/**
 * Create a metatool instance.
 *
 * Assembles: store + procedures + primitives + overlays → merged API + eval sandbox.
 */
export async function createMetatool(options: CreateMetatoolOptions): Promise<MetatoolInstance> {
  ...
  // 2. IO primitives backed by the configured MetatoolRuntime
  const io: IoApi = createIoApi(cwd, fsLayer, runtimeLayer)
  ...
  const overlayOps: Record<string, Function> = {
    loadOverlay: async (overlay: MetatoolOverlay): Promise<void> => { await overlayManager.load(overlay) },
    unloadOverlay: async (id: string): Promise<void> => { await overlayManager.unload(id) },
    switchOverlay: async (overlay: MetatoolOverlay): Promise<void> => { await overlayManager.switchTo(overlay) },
    overlays: () => overlayManager.active(),
    hasOverlay: (id: string) => overlayManager.has(id),
  }

  // 8. Convert legacy plugins → overlays
  const allOverlays: MetatoolOverlay[] = [
    ...overlayInputs,
...
export type { MetatoolHost, RenderAdapter, HostContext, SteerFormatter } from "./host.js"
export { createTestHost } from "./host.js"

// Legacy compat
export type { MetatoolPlugin, PluginLoadResult } from "./plugin.js"
export { mergePlugins } from "./plugin.js"

export type { MetatoolCore, MetatoolConfig, MetatoolInstance } from "./types.js"
export { MetatoolRuntime, makeNodeRuntimeLayer } from "./services/MetatoolRuntime.js"
export type { MetatoolRuntimeShape, RuntimeProcess } from "./services/MetatoolRuntime.js"
export {
  RuntimeShellCommand,
  RuntimeArgvCommand,
  RuntimeCommand,
...
export type {
  RuntimeShellCommand as RuntimeShellCommandType,
  RuntimeArgvCommand as RuntimeArgvCommandType,
  RuntimeCommand as RuntimeCommandType,
  RuntimeExecOptions as RuntimeExecOptionsType,
  RuntimeExecResult as RuntimeExecResultType,
  RuntimeError as RuntimeErrorType,
  RuntimeSpawnOptions as RuntimeSpawnOptionsType,
  RuntimeProcessDescriptor as RuntimeProcessDescriptorType,
} from "./schemas/runtime.js"
