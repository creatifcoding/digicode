 ...
 *
 * IMPORTANT: this adapter uses node:fs/promises. The metatool extension runs
...
const impl = FS.make({
  ...
  stat: (path) =>
    tryFs("stat", async () => {
      ...
      return {
        ...
        blksize: Option.some(FS.Size(s.blksize)),
  ...
  copy: (from, to) =>
    tryFs("copy", async () => { await NFS.cp(from, to, { recursive: true }) }),
  ...
  truncate: (path, len) =>
    tryFs("truncate", async () => { await NFS.truncate(path, Number(len ?? 0)) }),
  ...
  makeTempDirectory: (opts) =>
    tryFs("makeTempDirectory", async () => await NFS.mkdtemp(Path.join(opts?.directory ?? OS.tmpdir(), opts?.prefix ?? 'rlm-'))),

  makeTempDirectoryScoped: (opts) =>
    Effect.acquireRelease(
      tryFs("makeTempDirectoryScoped", async () => await NFS.mkdtemp(Path.join(opts?.directory ?? OS.tmpdir(), opts?.prefix ?? 'rlm-'))),
      (dir) => Effect.catchIf(tryFs("removeTempDirectoryScoped", async () => { await NFS.rm(dir, { recursive: true, force: true }) }), () => true, () => Effect.void),
    ),
  ...
  makeTempFileScoped: (opts) =>
    Effect.acquireRelease(
      ...
      (file) => Effect.catchIf(tryFs("removeTempFileScoped", async () => { await NFS.rm(file, { force: true }) }), () => true, () => Effect.void),
    ),
