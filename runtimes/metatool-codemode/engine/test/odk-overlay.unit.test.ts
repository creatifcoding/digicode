 ...
 * The ODK overlay validates itself and the metatool overlay.
 */
...
const odkov = odk(() => [odkov, metatool])
...
describe("ODK overlay: methods", () => {
  ...
  it("validate method works", () => {
    const r = odkmethods.validate(metatool)
    expect(r).toBeDefined()
    expect(r.valid).toBe(true)
  })

  it("conformance method works", () => {
    const r = odkmethods.conformance(metatool)
    expect(r.level).toBeGreaterThanOrEqual(1)
  })

  it("inspect method works", () => {
    const r = odkmethods.inspect(metatool)
  ...
  it("catalog returns loaded overlays", () => {
    ...
    expect(entries.some((e: any) => e.id === "metatool")).toBe(true)
  })
  ...
  it("testHarness generates test code", () => {
    const test = odkmethods.testHarness("metatool", "Skill Governance", "governance", [
      { name: "discover", arity: 0 },
      { name: "inspect", arity: 1 },
    ])
    expect(test).toContain("metatool")
...
// ── Metatool validation via ODK ─────────────────────────────────

describe("ODK: metatool dogfood", () => {
  it("metatool passes validate", () => {
    const r = validate(metatool)
    expect(r.valid).toBe(true)
  })

  it("metatool conformance ≥ 1", () => {
