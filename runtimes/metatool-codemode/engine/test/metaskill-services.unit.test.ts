/**
 * @module test/metatool-services.unit
 *
...
import {
  ...
  FreshnessService,
  profile, each, where,
} from '../src/plugins/metatool-services/index.js'
...
function setupTestWorkspace(): string {
  ...
  // Create metatool dir with protocols
  const msDir = join(skillsDir, 'metatool')
...
describe('Metatool Effect v4 Services', () => {
  let tmpDir: string
  let runtime: ReturnType<typeof ManagedRuntime.make>

  beforeAll(() => {
    tmpDir = setupTestWorkspace()
    const layer = makeMetatoolLayer(tmpDir).pipe(
  ...
  describe('SkillDiscovery', () => {
    it('discovers skills sorted alphabetically', async () => {
      ...
      expect(skills.length).toBe(3) // alpha, beta, metatool
      expect(skills[0].name).toBe('alpha')
      expect(skills[1].name).toBe('beta')
      expect(skills[2].name).toBe('metatool')
    })
  ...
  describe('FrontmatterService', () => {
    it('reads frontmatter from skill docs', async () => {
      ...
      expect(fm['SKILL.md']['governed-by']).toBe('metatool')
    })
  })
