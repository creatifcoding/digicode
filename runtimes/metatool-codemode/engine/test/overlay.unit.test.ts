...
import { OverlayManager, pluginToOverlay, type MetatoolOverlay } from '../src/overlay.js'
import type { MetatoolCore } from '../src/types.js'

// ── Helpers ──────────────────────────────────────────────────────

function makeOverlay(id: string, opts: Partial<MetatoolOverlay> = {}): MetatoolOverlay {
  return {
    id,
    name: `Overlay ${id}`,
...
function makeMockCore(): MetatoolCore {
  return {
    store: {} as any,
    procedures: {
      describe: vi.fn(async () => null),
      define: vi.fn(async (name: string, fn: Function, opts: any) => ({
        name,
        code: fn.toString(),
...
describe('OverlayManager — compilation', () => {
  ...
  it('seeds procedures idempotently by code, manifest, and tags', async () => {
    const stored = new Map<string, any>()
    const define = vi.fn(async (name: string, fn: Function, opts: any) => {
      const record = { name, code: fn.toString(), manifest: opts.manifest, tags: opts.tags }
      stored.set(name, record)
      return record
    })
    const core = makeMockCore()
    ;(core.procedures as any).describe = vi.fn(async (name: string) => stored.get(name) ?? null)
    ;(core.procedures as any).define = define
    const procedure = function seededProcedure(_mt: unknown, value: unknown) { return value }
    const overlay = makeOverlay('seed', {
      procedures: [{ name: 'seededProcedure', fn: procedure, manifest: 'mt.fn.seededProcedure(value)', tags: ['digicanvas'] }],
    })

    const first = new OverlayManager()
    first.setCore(core)
    await first.load(overlay)
    expect(define).toHaveBeenCalledTimes(1)

    const second = new OverlayManager()
    second.setCore(core)
    await second.load(overlay)
    expect(define).toHaveBeenCalledTimes(1)
    expect(stored.get('seededProcedure').tags).toEqual(['overlay:seed', 'digicanvas'])
  })
