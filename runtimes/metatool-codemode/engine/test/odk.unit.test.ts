 ...
 * Tests run against both synthetic overlays and the live metatool overlay.
 */
...
import { metatoolPlugin } from "../src/plugins/metatool.ts"
import { NodeFileSystemLayer } from "../src/adapters/filesystem-node.ts"
import type { MetatoolOverlay } from "../src/overlay.ts"

// ── Fixtures ─────────────────────────────────────────────────────

const minimal: MetatoolOverlay = {
  id: "test-minimal",
...
const complete: MetatoolOverlay = {
  id: "test-complete",
...
const metatool = metatoolPlugin(process.cwd(), NodeFileSystemLayer)
...
describe("ODK: validate", () => {
  ...
  // ── Dogfood: metatool overlay ──
  it("metatool overlay passes validation (0 errors)", () => {
    const r = validate(metatool)
    expect(r.valid).toBe(true)
    expect(r.errors.length).toBe(0)
  })
})
...
describe("ODK: conformance", () => {
  ...
  // ── Dogfood: metatool ──
  it("metatool → level ≥ 1 (valid)", () => {
    const r = conformance(metatool)
...
describe("ODK: inventoryFacets", () => {
  ...
  it("metatool has ≥ 4 facets", () => {
    const f = inventoryFacets(metatool)
...
describe("ODK: introspectMethods", () => {
  ...
  it("metatool methods all have function type", () => {
    const methods = introspectMethods(metatool)
...
describe("ODK: compare", () => {
  ...
  it("partial overlap → collision detection", () => {
    const overlapping: MetatoolOverlay = {
