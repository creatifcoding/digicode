/**
 * @module test/metatool-plugin.unit
 *
 * Tests for the metatool domain plugin.
...
const cwd = mkdtempSync(join(tmpdir(), 'metatool-plugin-test-'))
...
describe('metatoolPlugin', () => {
  const plugin = metatoolPlugin(cwd, NodeFileSystemLayer)

  it('has correct id and name', () => {
    expect(plugin.id).toBe('metatool')
  ...
  it('has guide sections', () => {
    ...
    expect(plugin.guide!.sections[0].id).toBe('metatool-ops')
