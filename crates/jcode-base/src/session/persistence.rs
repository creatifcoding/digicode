...
fn snapshot_holds_messages(snapshot_bytes: &[u8]) -> bool {
    ...
    let Some(pos) = snapshot_bytes.windows(KEY.len()).position(|w| w == KEY) else {
        return false;
    };
...
impl Session {
    ...
    fn checkpoint_snapshot(&mut self, snapshot_path: &Path, journal_path: &Path) -> Result<()> {
        ...
        // The in-memory persistence vector is a second, cheaper signal that
        // this checkpoint would destroy a transcript. Preserve upstream's
        // pre-wipe recovery copy as well as the on-disk content guard above.
